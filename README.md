...
Windows 本地也可以直接双击 `KnowledgeGovernanceConsole.exe`，或双击 `start_knowledge_console.bat`。两种方式都会读取 `config.yaml` 中的 `launch_host`、`launch_port` 和 `launch_timeout_seconds`，如果端口被占用会自动尝试后续端口；启动记录会写入 `launcher.log` 便于排查。
